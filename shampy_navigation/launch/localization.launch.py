...
def generate_launch_description():
    pkg_shampy_navigation = get_package_share_directory('shampy_navigation')
    pkg_nav2_bringup = get_package_share_directory('nav2_bringup')

    localization_params_arg = DeclareLaunchArgument(
        'params',
        default_value=PathJoinSubstitution(
            [pkg_shampy_navigation, 'config', 'localization.yaml']),
        description='Localization parameters')

    map_arg = DeclareLaunchArgument(
        'map',
        default_value=PathJoinSubstitution(
            [pkg_shampy_navigation, 'maps', 'warehouse.yaml']),
